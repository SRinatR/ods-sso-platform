...
Assert-Pattern "SEC-BASE-010" $identityService '(?s)val existing = users\.findByTenantIdAndEmailIgnoreCase\(tenant\.id, email\).*if \(existing != null\) \{.*sendVerification\(.*return properties\.requireEmailVerification' "duplicate registration must retain the generic email-code response"
