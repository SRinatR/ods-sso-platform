...
$identityDtos = "backend-kotlin/src/main/kotlin/uz/ods/sso/identity/IdentityDtos.kt"
...
Assert-Absent "SEC-BASE-004" $securityConfig 'HttpSessionSecurityContextRepository' "authorization server must not restore first-party identities from HttpSession"
Assert-Absent "SEC-BASE-004" $securityConfig 'SessionCreationPolicy\.STATELESS' "authorization server must not globally disable framework session mechanics"
...
Assert-Pattern "SEC-BASE-010" $identityService '(?s)val existing = users\.findByTenantIdAndEmailIgnoreCase\(tenant\.id, email\).*if \(existing != null\) \{.*sendVerification\(.*return true' "duplicate registration must retain the generic email-code response"
Assert-Pattern "SEC-BASE-010" $identityService 'REGISTRATION_EXISTING.*REGISTRATION_REPEATED|REGISTRATION_REPEATED.*REGISTRATION_EXISTING' "duplicate registration must not disclose whether the email is already verified"
Assert-Absent "SEC-BASE-010" $identityService 'EMAIL_VERIFICATION_SKIPPED|consumeVerificationToken|VerifyEmailRequest\(token' "email verification must not keep legacy link-token paths"
Assert-Absent "SEC-BASE-010" $identityDtos 'verificationRequired' "registration response must not keep the old verification_required flag"
